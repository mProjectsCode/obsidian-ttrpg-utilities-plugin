import {equalOrIncludes, isFalsy, TTRPGUtilitiesInternalError} from './Utils';
...
export class EnclosingPair {
	...
	constructor(openingString: string, closingString?: string) {
		if (isFalsy(openingString)) {
			throw new TTRPGUtilitiesInternalError('opening string must not be empty');
		}
...
export class ParserUtils {
	...
	static split(str: string, separator: string, ignore?: EnclosingPair): string[] {
		if (!str) {
			throw new TTRPGUtilitiesInternalError('string must not be empty');
		}
		if (!separator) {
			throw new TTRPGUtilitiesInternalError('separator must not be empty');
		}
	...
	static isStringAt(str: string, subStr: string, index: number): boolean {
		if (index < 0) {
			throw new TTRPGUtilitiesInternalError('index must be greater than 0');
		}
		if (index >= str.length) {
			throw new TTRPGUtilitiesInternalError('index out of bounds of string');
		}
		if (!str) {
			throw new TTRPGUtilitiesInternalError('string must not be empty');
		}
		if (!subStr) {
			throw new TTRPGUtilitiesInternalError('sub string must not be empty');
		}
